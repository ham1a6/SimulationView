...
// データ全体(対角線約785km)が画面に収まる距離(400,000m)より十分外側まで縮小できるよう、
// 2,000,000mまでズームアウトできる(データ周辺の海域を含めた広域を俯瞰できる)。
...
//
// 「海の描画を一律の平面ではなくデータ外接矩形の外側だけにしたい」との要望を受けて
// 背景スカート(`terrain::mesh::append_background_skirt`)を額縁状にした際、その外周
// (`BACKGROUND_OUTER_RADIUS_M`)がこのZ_FARを超えるとカメラの遠方クリップ面で切り取られ、
// 何も描画されない黒い背景(sky/clear color)が視野の一部に見えてしまう不具合が実機で
// 見つかった。実地形が実際に必要とする範囲(MAX_DISTANCE+対角線長)はこれまで通りだが、
// 反転Zの深度精度は主にnear値側の設定で決まり、z_farをこの範囲より先まで伸ばしても
// (=どのみち実地形が存在しない領域を追加するだけなので)実地形側の精度には影響しない。
// そのため背景スカートの外周を十分覆えるよう、実地形が必要とする範囲より大きい値にしてある。
// スカートの外周(BACKGROUND_OUTER_RADIUS_M=4,000,000m、隅のコーナーウェッジはその2倍の
// 8,000,000mまで伸ばす)+MAX_DISTANCE(2,000,000m)に対して十分な余裕を持たせた値。
const Z_FAR: f32 = 20_000_000.0;

...
pub fn add_demo(state: DrawingState, lat: f64, lon: f64) -> Vec<DrawingId> {
    ...
    let at = |dlat: f64, dlon: f64, altitude: Altitude| {
        Position::world(lat + dlat, lon + dlon, altitude)
    };
    ...
    add(
        Shape::Circle {
            center: at(0.0, 0.0, ground),
            radius: 8_000.0,
        },
        Style::fill_and_stroke(
            Color::rgba(0.2, 0.9, 0.3, 0.35),
    ...
    add(
        Shape::Sector {
            center: at(0.0, 0.0, Altitude::Msl(4_500.0)),
            radius: 40_000.0,
            start_deg: 0.0,
            end_deg: 90.0,
        },
        Style::fill_and_stroke(
            Color::rgba(1.0, 0.3, 0.2, 0.25),
            Color::rgb(1.0, 0.5, 0.4),
            2.0,
        ),
    );
    // 海抜3000mの水平な矩形(30度回転)。
    add(
        Shape::Rect {
            center: at(-0.15, 0.2, Altitude::Msl(3_000.0)),
            width: 20_000.0,
            height: 12_000.0,
            rotation_deg: 30.0,
        },
    ...
    add(
        Shape::Polygon {
            points: l_shape
                .iter()
                .map(|&(dlat, dlon)| at(0.1 + dlat, -0.35 + dlon, ground))
                .collect(),
        },
    ...
    // ---- 絶対座標: 3D図形(不透明) ----
    let solid =
        Style::fill_and_stroke(Color::rgb(0.85, 0.85, 0.9), Color::rgb(0.1, 0.1, 0.15), 1.5);
    add(
        Shape::Sphere {
            center: at(0.12, 0.1, Altitude::Msl(5_000.0)),
            radius: 2_500.0,
        },
        solid,
    );
    add(
        Shape::Cuboid {
            base_center: at(0.12, 0.2, ground),
            size_m: [4_000.0, 2_500.0, 5_000.0],
            heading_deg: 30.0,
        },
    ...
    add(
        Shape::Cylinder {
            base_center: at(0.02, 0.2, ground),
            radius: 1_500.0,
            height: 6_000.0,
        },
    ...
    add(
        Shape::Cone {
            base_center: at(0.02, 0.3, Altitude::AboveGround(500.0)),
            radius: 2_000.0,
            height: 7_000.0,
        },
        Style::fill_and_stroke(
            Color::rgba(0.8, 0.4, 0.9, 0.5),
    ...
    add(
        Shape::Polyline {
            points: route
                .iter()
                .map(|&(dlat, dlon)| at(dlat, dlon, Altitude::AboveGround(200.0)))
                .collect(),
        },
    ...
    add(
        Shape::Polyline {
            points: vec![
                Position::screen(Corner::Center, -12.0, 0.0),
                Position::screen(Corner::Center, 12.0, 0.0),
            ],
        },
        cross,
    ...
    add(
        Shape::Polyline {
            points: vec![
                Position::screen(Corner::Center, 0.0, -12.0),
                Position::screen(Corner::Center, 0.0, 12.0),
            ],
        },
        cross,
    ...
    add(
        Shape::Circle {
            center: Position::screen(Corner::BottomRight, -60.0, -60.0),
            radius: 36.0,
        },
        Style::fill_and_stroke(
    ...
    add(
        Shape::Sphere {
            center: Position::view(-8_000.0, -22_000.0, 100_000.0),
            radius: 6_000.0,
        },

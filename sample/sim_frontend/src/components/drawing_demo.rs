...
use sim3dview::terrain::drawing::{
    Altitude, Color, Corner, DrawingState, Position, Shape, Style,
};

/// 原点(`lat`,`lon`)のまわりにデモの図形を追加する。既存の図形は消さない。
pub fn add_demo(state: DrawingState, lat: f64, lon: f64) {
    ...
    state.add(Shape::Sphere { center: at(0.12, 0.1, Altitude::Msl(5_000.0)), radius: 2_500.0 }, solid);
    state.add(

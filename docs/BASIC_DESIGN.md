...
- 地形データはシミュレーション状態とは独立した静的データとして配信する。起動時に最粗レベルを読み込み、
  カメラ位置に応じて必要なLODチャンクを追加取得・差し替えする
...
| GDAL | GeoTIFF前処理(タイルごとの読み込み・ダウンサンプリング) | MIT/X11系ライセンス(ドライバ次第) |
...
| 7 | 地形エリアの規模 | `map_data`内の**全タイル**(現在390タイル)を対象にする。外接矩形は前処理時にタイル構成から自動計算する |
...
| 9 | メッシュ解像度 | **1度タイル単位のLOD+近いタイルは6x6チャンク**(全タイルを最粗でも約620m/セルのチャンクで常駐し、カメラに近いチャンクほど185m/62m/31m(元データの30m)のレベルを取得して差し替える。チャンクの頂点は合計2500万まで。見えているタイルを先に配り、見えていないタイルの下限はその残り。DETAILED_DESIGN.md 2.4節・6.10節) |
...
基本機能と追加機能は実装済みである。現行ライブラリを構成する機能、再実装する場合の順序、
各段階の受け入れ基準は[IMPLEMENTATION_GUIDE.md](IMPLEMENTATION_GUIDE.md)を正とする。
個々の機能が現在の方式へ至った経緯や、廃止した旧方式は
[DEVELOPMENT_HISTORY.md](DEVELOPMENT_HISTORY.md)にのみ残す。

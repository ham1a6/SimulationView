...
- [ ] ドームは方位を間引く(高いリングほど多く)。値は平滑化せず生の計算結果(方位ごとの遮蔽判定)をそのまま使う。間引きの違うリングの間は扇状の三角形でつなぐ(T字の継ぎ目を作らない)。
- [ ] 複数の覆域の同時表示: 計算・キャッシュ・ジオメトリは観測点ごと、GPUへは表示する観測点のジオメトリをつないで1バッファ。観測点ごとに色を変える。
- [ ] 断面図は、選択中のシンボルの位置が中心(無ければ基準位置)。シンボルの移動で毎回作り直さない(中心を約500mに丸めたキーの変化と、追跡しない読み取りで刻む)。

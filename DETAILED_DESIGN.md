...
**計算コスト**: 観測点1つあたり6400方位角 × 200サンプル/方位角 = 128万回の`heightmap`双線形補間
(3D描画・極座標図とも選択中の観測点のみ計算するため、観測点の総数には比例しない)。
方位角を1度刻み(360)から1mil刻み(6400)にした際に約17.8倍になった。リリースビルド(`trunk serve --release`)の
ブラウザ実測で、観測点のパラメータを1回編集したときの再計算+再描画が、3D表示で約120ms
(覆域ドーム約80ms+極座標図約40ms)、2D表示で約50ms(360方位角のときは数ms)。
同じ計算はカメラ操作による地形のLOD切り替え(標高が変わる)のたびにも走る(`rebuild_markers`、選択中の
観測点が3Dドームのときは約80ms)。速度対策として`EnuTransform::inverse`の曲率半径を`new`で前計算し
(呼び出しごとの三角関数・平方根を削減、ネイティブ計測で約3割減)、2D覆域の塗りと輪郭線で
`compute_coverage_area`の結果を共有している。

...
- **描き方(タイルごと)**: 遠いタイルは**タイル全体を1枚のメッシュ(レベル0、約1.85km/セル、約4千頂点)**で描く。
  全タイルのレベル0(合計約155万頂点)は起動時に`base.bin`から作って常駐する。カメラに近づいてレベル0では
  粗すぎるタイルは、**6×6のチャンクのメッシュ**に切り替え、チャンクごとに独立してレベル(1〜4)を持つ
...
- **計画**(`terrain::lod::plan_levels`。GPU・ネットワークに触れない純粋関数):
  ...
  - チャンクのレベルは、画面(CSSピクセル)上で**1セルが`CHUNK_TARGET_CELL_PX`(1ピクセル)以下**になる最も粗い
    ...
    視点から約46km以内)。2ピクセルだった頃(`TARGET_CELL_PX`のまま)は約23km以内で、それより遠い中距離は
    62m/185mで描かれていた
  - チャンク表示へ切り替える条件は「レベル0では`TARGET_CELL_PX`(2ピクセル)以下を満たさない」(チャンクの
    `CHUNK_TARGET_CELL_PX`とは別の定数。切り替え条件まで細かくすると、遠くの多数のタイルが一斉にチャンク化して
    最低限の分(1タイル約3.5万頂点)で予算を食い、近くの細かさに回らなくなる)、全体表示へ戻す条件は
    「レベル0で足りる距離の0.7倍まで離れた」(ヒステリシス)。チャンクのレベルは、理想より1つだけ
    細かい現状のレベルなら下げない。視野の外は現状維持
  - チャンクの頂点の合計は`DETAIL_VERTEX_BUDGET`(600万)以下にする。まず、チャンク表示にしたいタイルに
    全チャンクをレベル1で載せる最低限の分を確保し(足りなければそのタイルは全体表示のまま)、
    次に、見えていて近いチャンクから順に、目標レベルまで予算の許す限り上げる。最細のチャンクは
    約36万頂点なので、同時に16個程度まで(常駐の頂点は最大で約755万。従来の単一メッシュは約420万。
    予算を300万から倍にしたのは、レベル選択を細かくしたぶん細かいレベルが必要な範囲が広がったため)。
    予算が足りない遠い側のチャンクは、目標より粗いレベルのままになる(近い側が優先)
- **適用**(`ui::terrain_view`の`schedule_lod`→150ms待ってから`update_lod`。操作が続く間はまとめ、
  ...
  (レベル1→2→3→4と段階的に細かくなる)。1回の更新で作る頂点数には上限(100万)があり、残りは次の更新へ
- **取得**: レベル2以下はタイル1枚分のファイル(小さい)をまとめて取得してチャンクごとに分割し、
  レベル3以上はHTTP Rangeでチャンク1個分だけ取得する(最細で約0.7MB)。同時6件まで、失敗は再試行しない。

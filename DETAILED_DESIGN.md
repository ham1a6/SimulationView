...
**覆域は選択中の観測点についてのみ**描く(複数観測点の覆域を同時に重ねると見づらいため)。
原点変更時(メッシュ再構築後)・観測点の追加/削除/選択変更のたびに頂点データを作り直す。

**覆域の3D表示は半球状のワイヤーフレーム(ドーム)**(要望により、地表の起伏に沿わせた
単なる等高線的リングから変更): `terrain::markers::push_coverage_dome`が、`compute_los`の
出力(各方位角の見通し限界距離)を**その方位角でのドーム半径**とみなし、仰角0°(地表)
〜80°(`DOME_RING_ELEVATIONS_DEG = [0,20,40,60,80]`)の5段の緯度リング(円周、各360分割)+
12本の経度線(縦線、地表〜最上段リングを結ぶ)で構成する。各リングの点は単純に
`horizontal = range_m * cos(仰角), height = observer_height + range_m * sin(仰角)`で
extrudeするだけ(地形の凹凸は一切参照しない)ため、地表の起伏に関わらず滑らかな半球状の
見た目になる。仰角90°(真頭上)は方位角ごとに半径が異なり単一の頂点に収束しないため、
リングは80°までとし頂点(アペックス)は作らない。

...
- **UI**: `ui::drawing_editor::DrawingEditor`(ツールボタン・新規図形の見た目/高度・一覧・編集フォーム)。地図をクリックできるよう、モーダルの
  `FloatingPanel`ではなくタブの中身として置く(サンプルは右パネル上段の**「作図」タブ**)。作成中の案内と`確定`/`1つ戻す`/`終了`は、
...
消すときは自分が追加した図形のIDだけを消すので、ユーザーが作った図形は残る)がある。図形を自分で作る操作は、右パネル上段の「作図」タブ(6.11節「図形の対話作成」)。

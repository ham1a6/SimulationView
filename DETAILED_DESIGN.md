...
低地(t=0)の色は当初`(0.05, 0.35, 0.35)`(深緑がかった青緑)だったが、`WATER_COLOR`
`(0.55, 0.78, 0.92)`と同じ青緑系統の色相だったため、低地の陸地(NaNセルと隣接して
細かく入り組む沿岸部に多い)が遠景でMSAA/スーパーサンプリングによって周囲の海色と
混ざり、実際には陸地データが存在するにもかかわらず海にしか見えなくなる問題があった
(「原点から遠いところでは陸地があるはずなのに海になっている」との報告を受けて
heightmap.binを直接検証し、位置・標高・NaN判定はすべて正しく色のコントラスト不足が
原因と判明)。水色から明確に離れた緑系の色相`(0.12, 0.4, 0.18)`に変更し解消した。
...
- `build_mesh()`が、実データの頂点群とは別に、`WATER_COLOR`で塗った巨大な水平の板
  ...
  遠方クリップ距離(`camera.rs`のMAX_DISTANCE=2,000,000m/Z_FAR=20,000,000m)より十分大きく、

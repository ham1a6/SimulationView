...
- 中段: `MID_ROWS`(4)行×`MID_TOTAL_COLS`(8)列のダミーボタン。可視幅は`cols`列ぶんしかないため、
  残りの列は`.vab-scroll-outer`(`overflow-x: auto`)による**横スクロールで操作できる**
  (中身の`.vab-mid-grid`に`min-width: calc(100% * MID_TOTAL_COLS / cols)`を指定し、可視列より
  widthを広げることでスクロールを発生させている)

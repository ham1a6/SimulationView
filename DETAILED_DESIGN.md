...
- **計画**(`terrain::lod::plan_levels`。GPU・ネットワークに触れない純粋関数):
  ...
  - 画面(CSSピクセル)上で**1セルが`TARGET_CELL_PX`(2ピクセル)以下**になる最も粗いレベルを理想とする
    (ピクセルの実寸は透視投影では`2*距離*tan(fov/2)/canvas高さ`、正射影では`表示高さ/canvas高さ`)。
    30mのレベル4が使われるのは、1ピクセルが約31m未満になる距離(canvas高さ700pxで視点から約23km以内)
  - チャンク表示へ切り替える条件は「レベル0では理想を満たさない」、全体表示へ戻す条件は
    「レベル0で足りる距離の0.7倍まで離れた」(ヒステリシス)。チャンクのレベルは、理想より1つだけ
    細かい現状のレベルなら下げない。視野の外は現状維持
  - チャンクの頂点の合計は`DETAIL_VERTEX_BUDGET`(300万)以下にする。まず、チャンク表示にしたいタイルに
    全チャンクをレベル1で載せる最低限の分を確保し(足りなければそのタイルは全体表示のまま)、
    次に、見えていて近いチャンクから順に、目標レベルまで予算の許す限り上げる。最細のチャンクは
    約36万頂点なので、同時に8個程度まで(常駐の頂点は最大で約455万。従来の単一メッシュは約420万)

...
- **描き方**: 画面いっぱいの三角形1枚(`vs_fullscreen`)を、地形メッシュより**先に**、**深度を書かず・テストもせず**に描く
  (`water_pipeline`)。フラグメント(`fs_water`)が各画素の視線と楕円体の交点の有無を求め、あれば水色、
  なければ`discard`(空は黒のまま)。地形メッシュは深度テストに関係なく水域の上に上書きされるので、
  **地形の標高が0m以下(DSMの負の値・海面以下の陸地)でも水域が地形を隠すことはない**(標高と深度を
  比較しないため、構造的に起きない)。ドーム・マーカーは地形の深度でテストされるので水域の影響を受けない

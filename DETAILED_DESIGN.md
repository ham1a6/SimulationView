...
| `base.bin` | レベル0(タイル全体で1枚)を全タイル分、`tile_index.json`の順に連結したもの | int16, リトルエンディアン。約3.3MB。フロントが起動時に全部取得して常時保持する |
...
- **描き方(タイルごと)**: 遠いタイルは**タイル全体を1枚のメッシュ(レベル0、約1.85km/セル、約4千頂点)**で描く。
  全タイルのレベル0(合計約165万頂点)は起動時に`base.bin`から作って常駐する。カメラに近づいてレベル0では
...
- **計画**(`terrain::lod::plan_levels`。GPU・ネットワークに触れない純粋関数):
  ...
  - チャンクの頂点の合計は`DETAIL_VERTEX_BUDGET`(300万)以下にする。まず、チャンク表示にしたいタイルに
    ...
    約36万頂点なので、同時に8個程度まで(常駐の頂点は最大で約465万。従来の単一メッシュは約420万)

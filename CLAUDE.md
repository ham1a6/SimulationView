...
上記2文書に完全に統合済みで、両ファイルは近く削除される予定(現時点ではまだリポジトリに残っている)。
削除後に過去の意思決定の経緯そのもの(なぜその結論に至ったか)を辿りたい場合はgit履歴を参照する。
...
入力値をクライアント側で検証し(範囲外なら`設定`ボタンを無効化、DESIGN.md 3.5節)、`set_origin`
...
### 未検証: スプリッタードラッグ時のリサイズ追従

`TerrainRenderer::resize()`を`ResizeObserver`のコールバックから呼ぶ実装は入れてあるが、**この
セッションでは動作確認できていない**。原因はコード側ではなく、作業に使っていたBrowserペインが
非表示(バックグラウンド)状態になっており、Chromiumがそのタブの`ResizeObserver`通知自体を
スロットリング(完全停止)していたため(独立に作成したテスト用の素のJS `ResizeObserver`も
一切発火しないことを確認して切り分け済み)。実際に使う際にBrowserペインを表示した状態で
スプリッターをドラッグし、地形が追従してリサイズされるか確認すること。

...
検討過程のメモ: 当初は「地形マスクの影響を仰角ごとに正しく計算し直す」物理的に厳密な
モデル(観測点からの直線が仰角一定のまま地形にぶつかる距離を仰角ごとに求める)を検討したが、
地形にほとんど遮蔽されない開けた方角では仰角が上がるほど水平距離がほぼ変わらないまま
高さだけが`tan(仰角)`で急増し、ドームというより「筒」や「扇」に近い、要望とは違う見た目に
なることに気づいた。「半球状のオブジェクトに見せたい」という視覚的な要望を優先し、
物理的な厳密さより「見た目が綺麗な半球になる」sin/cos展開のほうを採用した。

ブラウザ実機で確認済み: マーカーを1つ配置すると地表から放射状に広がる水色のワイヤーフレーム
ドームが表示される、俯瞰プリセットから浅い角度までカメラを回転させると(黒空を背景に)
明確に半球のシルエットとして見える、をスクリーンショットで確認。

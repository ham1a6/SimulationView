...
| 知りたいこと | 参照先 |
|---|---|
| システム概要・設計方針・詳細仕様・再実装手順 | [docs/DETAILED_DESIGN.md](docs/DETAILED_DESIGN.md)(新しいセッションはまず0節。実装仕様は9節、再実装は10節) |
...
| ライブラリを1から再実装したい(エージェントへの実装依頼を含む) | [docs/DETAILED_DESIGN.md](docs/DETAILED_DESIGN.md) 10節(フェーズ・受け入れ基準)と9節(仕様の要点) |
...
docs/                 # 統合設計書(9節=実装仕様、10節=再実装ガイド)・開発履歴。索引はdocs/README.md
...
ライブラリ/サンプルの責務分割の詳細はdocs/DETAILED_DESIGN.md 0.4節・6節冒頭の対応関係表を参照。
...
- 地形は**1度タイル単位のLOD+近いタイルは6x6チャンク**: 全タイルをチャンクで常駐し(最も粗くても
  ...
  `ui/terrain_view/lod_driver.rs`が適用。チャンクの頂点は合計2500万まで(見えているタイルの下限→レベルごとの周回の順に配り、見えていないタイルはその残り)。画面外のメッシュは

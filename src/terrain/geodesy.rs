...
impl Ellipsoid {
    /// WGS84(長半径6,378,137m、逆扁平率298.257223563)。地形データを読む前に楕円体が必要な
    /// 場面(作図の距離・方位の計算など)と、単体テストで使う。地形の座標変換には、この定数ではなく
    /// 地形データの`metadata.json`の`ellipsoid`を使う(実データはGRS80の逆扁平率298.257222101。
    /// WGS84との差は短半径で約0.1mmで、表示・計算に影響しない)。

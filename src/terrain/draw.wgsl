...
// 観測点のマーカー・2Dの覆域・航跡のシンボル(画面サイズ固定のビルボード)もこのシェーダーで描く。
// 頂点データは`terrain::vertex::DrawVertex`。座標の種類(絶対座標・視点空間・画面)ごとに
// `view_proj`を差し替えて、同じシェーダーで描く。
// vs_mainは頂点の種類(params.z)で、向きつきビルボード→ビルボード→面→線の順に分岐する。

struct DrawUniform {
    // 頂点の座標→クリップ座標(座標の種類ごとに違う行列)。
